...
> 所有 `/admin/*` 页面与 `/api/admin/*` 均经鉴权中间件。
...
if path startsWith '/admin' or '/api/admin':
  if AUTH_MODE === 'access':
    jwt = headers['Cf-Access-Jwt-Assertion']
    verify AccessJWT(jwt) via Access public keys (jwks), check aud == AUD, exp
    fail → 401 (API) or 302 → /admin/login?... (page)
  else (token):
    cookie admin_token === env.ADMIN_TOKEN (constant-time) ?
    fail → 302 → /admin/login
  pass → next
```
Access JWT 校验:从 `https://<team>.cloudflareaccess.com/cdn-cgi/access/certs`(或 JWKS endpoint)取公钥,验证 RS256;`aud` 从 env `ACCESS_AUD` 取。降级 token 方案:`/admin/login` 表单提交 token,Set-Cookie `admin_token; HttpOnly; Secure; SameSite=Strict`。
...
```
所有 API 返回 JSON `{ok:true}` 或 `{ok:false,error}`。
...
COMMIT;
```
前台取规范标签:`WHERE alias_of_id IS NULL`。

## 审计日志(轻量,可选)

表 `audit_logs(id, action, target_id, meta TEXT, at TEXT)`(本任务迁移新增 `0003_audit.sql`)。关键操作写一行:`action` ∈ {approve,reject,edit,offline,republish,merge},`meta` 存 JSON(如 reject_reason、merge from/to)。后台可加 `/admin/audit` 查看页(可后置,不阻塞 MVP)。
...
- `ACCESS_TEAM`(如 `yourteam`),用于 JWKS URL
...
- 鉴权中间件是 Astro 全局 middleware,需确保只拦 `/admin` 与 `/api/admin`,不影响前台路由。
- 新增迁移 `0003_audit.sql`(若启用审计)需在 `foundation-db` 已应用基础上叠加。
...
- **审计表**:MVP 可先不落,但 design 已留位;若实现成本可控建议一并做,便于运营追责。
