...
1. **鉴权层**
   - `src/lib/auth/access.ts`:`verifyAccessJWT(jwt, env)`(取 JWKS、验签、aud、exp)。
   - `src/lib/auth/token.ts`:`verifyTokenCookie(cookie, env)`(常量时间比较)。
   - `src/middleware.ts`:按 `AUTH_MODE` 拦 `/admin` 与 `/api/admin`;失败 API→401,页面→302 `/admin/login`。
2. **登录页(token 模式)**
...
4. **状态迁移 API**
   - `approve.ts` / `reject.ts` / `offline.ts` / `republish.ts`:SQL 见 design.md;写审计(若启用)。
...
6. **标签归并**
   ...
   - `tags/merge.ts`:事务(见 design.md);写审计。
7. **审计(若启用)**
