...
- [x] 完成移动/平板/桌面三种视口的交互式浏览器验证；覆盖登录、预览、首次创建、数据库重复警告、确认后再次创建、成功编辑链接和移动端导航。不引入 Playwright。
...
- 应用内浏览器在 390x844、768x1024 和 1440x1000 三种视口完成验证：页面均无横向溢出；移动端预览表格限制在内部横向滚动容器；重复项未确认时提交按钮禁用；两次真实创建分别返回稳定编辑链接；应用控制台无错误。
